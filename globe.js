...
const props = {
    ...
    startingCountry: 'New York', // The key of the country to rotate the camera to during the introduction animation (and which country to start the cycle at)
    colours: {
        // Cache the colours
        globeDots: 'rgb(154, 157, 188)', // No need to use the Three constructor as this value is used for the HTML canvas drawing 'fillStyle' property
...
const getData = async () => {
    try {
        const results = await fetch('https://raw.githubusercontent.com/malikfaizanhaider/excelenergies/master/globe-points.json');
...
const addGlobe = () => {
    ...
    canvasGradient.addColorStop(.5, '#ffffff');
